                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.questions.update", [$question->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('answer') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="answer" id="answer" value="{{ old('answer', $question->answer) }}">
                        ...
                        <div class="form-group {{ $errors->has('language') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="language" id="language">
                                ...
                                @foreach(App\Models\Question::LANGUAGE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('language', $question->language) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
