        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.bookList.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.book-lists.update", [$bookList->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        {{-- <div class="form-group {{ $errors->has('uid') ? 'has-error' : '' }}">
                            <label class="required" for="uid">{{ trans('cruds.bookList.fields.uid') }}</label>
                            <input class="form-control" type="text" name="uid" id="uid" value="{{ old('uid', $bookList->uid) }}" required>
                            @if($errors->has('uid'))
                        ...
                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $bookList->title) }}">
                            @if($errors->has('title'))
                        ...
                        <div class="form-group {{ $errors->has('book_category') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="book_category_id" id="book_category_id">
                                @foreach($book_categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('book_category_id') ? old('book_category_id') : $bookList->book_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('book_tags') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="book_tags[]" id="book_tags" multiple>
                                @foreach($book_tags as $id => $book_tag)
                                    <option value="{{ $id }}" {{ (in_array($id, old('book_tags', [])) || $bookList->book_tags->contains($id)) ? 'selected' : '' }}>{{ $book_tag }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('language') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="language" id="language">
                                ...
                                @foreach(App\Models\BookList::LANGUAGE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('language', $bookList->language) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('year') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="year" id="year" value="{{ old('year', $bookList->year) }}" step="1">
                            @if($errors->has('year'))
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status">
                                ...
                                @foreach(App\Models\BookList::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $bookList->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
